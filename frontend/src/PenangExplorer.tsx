...
// ── Geoapify API Keys ─────────────────────────────────────────────────────────
const MAPTILES_KEY = import.meta.env.VITE_MAPTILES_KEY;
const ROUTING_KEY = import.meta.env.VITE_ROUTING_KEY;
const GEOCODING_KEY = import.meta.env.VITE_GEOCODING_KEY;
...
const FAMOUS_PLACES_FRONTEND = {
  ...
  hotel: [
    ...
    { name: 'Hard Rock Hotel Penang',        address: 'Jalan Batu Ferringhi, 11100 Penang',      lat: 5.467886434861278, lon: 100.24151277947169 },
...
export default function PenangExplorer({ onBack, onTripAdded, focusPlace }: PenangExplorerProps) {
  ...
  const [favouriteIds, setFavouriteIds] = useState<Record<string, string>>({});
  ...
  const placeKey = (place: Place) => place.name.trim().toLowerCase();

  const loadFavourites = async () => {
    try {
      const favourites = await tripService.getFavourites();
      const map: Record<string, string> = {};
      favourites.forEach(fav => {
        if (fav.destination) map[fav.destination.trim().toLowerCase()] = fav._id;
      });
      setFavouriteIds(map);
    } catch (err) {
  ...
  const togglePlaceFavourite = async (place: Place) => {
    const key = placeKey(place);
    const existingId = favouriteIds[key];
    setSavingFavourite(key);
    try {
      if (existingId) {
        await tripService.deleteTrip(existingId);
        setFavouriteIds(prev => { const next = { ...prev }; delete next[key]; return next; });
        showToast('Removed from favourites.');
      } else {
        const saved = await tripService.saveFavouritePlace(place);
        setFavouriteIds(prev => ({ ...prev, [key]: saved._id }));
  ...
  useEffect(() => {
    ...
    const categoryMap: Record<string, string> = {
      ...
      'Hotel':                  'hotel',
      'Beach':                  'beach',
      // Raw lowercase values from the API / Attractions tab
      'tourism':                'tourism',
      'restaurant':             'restaurant',
      'hotel':                  'hotel',
      'beach':                  'beach',
  ...
  function requestUserLocation() {
    ...
    setLocating(true);
    showToast('Searching current location...');
  ...
  const makeTripFromPlace = (place: Place): Omit<Trip, '_id'> => ({
    ...
    stops:       [{ name: place.name, address: place.address || '', lat: place.lat, lon: place.lon, category: place.type || '' }],
  });
  ...
  const handleAddTripSubmit = async (tripData: Omit<Trip, '_id'>) => {
    ...
    const budgetTotal = Number(tripData.budget?.total) || 0;
            ...
            {!loading && places.map((place, i) => (
              <div key={place.id || i} className={`place-card ${selectedPlace?.id === place.id ? 'selected' : ''}`} onClick={() => setSelectedPlace(place)}>
                ...
                <div className="card-actions">
                  ...
                  <button
                    className={`card-btn btn-favourite ${favouriteIds[placeKey(place)] ? 'active' : ''}`}
                    onClick={e => { e.stopPropagation(); togglePlaceFavourite(place); }}
                    disabled={savingFavourite === placeKey(place)}
                    title={favouriteIds[placeKey(place)] ? 'Remove from favourites' : 'Add to favourites'}
                  >
                    {savingFavourite === placeKey(place) ? '…' : favouriteIds[placeKey(place)] ? '❤️' : '🤍'}
                  </button>
                </div>
              </div>
            ))}
        ...
        <div className="map-wrapper">
          ...
          <MapContainer center={PENANG_CENTER} zoom={12} style={{ height: '100%', width: '100%' }} maxBounds={PENANG_BOUNDS} maxBoundsViscosity={0.8}>
            ...
            <FlyToLocation
              ...
              onDone={() => setShouldFlyToUser(false)}
            />
            ...
            {places.map((place, i) => (
              ...
              >
                <Popup>
                  ...
                  <button className={`popup-favourite-btn ${favouriteIds[placeKey(place)] ? 'active' : ''}`} onClick={e => { e.stopPropagation(); togglePlaceFavourite(place); }} disabled={savingFavourite === placeKey(place)}>
                    {favouriteIds[placeKey(place)] ? '❤️ Favourited' : '🤍 Favourite'}
                  </button>
                </Popup>
              </Marker>
            ))}
