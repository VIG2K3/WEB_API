...
export default function SmartRecommendations() {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      if (!GROQ_API_KEY) {
        throw new Error("Missing Groq API key. Add VITE_GROQ_API_KEY in your .env file.");
      }
      ...
      setIsTyping(false);
      const errorMessage = error?.message?.includes("Missing Groq API key")
        ? "❌ Missing Groq API key. Please add VITE_GROQ_API_KEY in your .env file."
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 900px) {
    .pb-card { min-height: 94px; }
  }

  @media (max-width: 720px) {
    .pb-card { padding: 12px; }
  }
`;
...
const st: Record<string, React.CSSProperties> = {
  ...
  card: {
    ...
    boxShadow: "none",
    animation: "pb-fadeUp 0.25s ease both",
    minHeight: 0,
  },
